...
def getUIContent():
   ...
   my_toolProperty.TitleText = "  Create lap shear coupon"
...
def getCustomToolPropertyContent():
   ...
   currRow += 1
   OverlapLbl = WPFControls.TextBlock()
   OverlapLbl.Text = "    Sheet overlap (mm):"
   WPFControls.Grid.SetRow(OverlapLbl, currRow)
   WPFControls.Grid.SetColumn(OverlapLbl, 0)
   
   global sheetOverlap
   sheetOverlap =WPFControls.TextBox()
   WPFControls.Grid.SetRow(sheetOverlap, currRow)
   WPFControls.Grid.SetColumn(sheetOverlap, 1)
   ...
   my_Grid.Children.Add(OverlapLbl)
   my_Grid.Children.Add(sheetOverlap)
